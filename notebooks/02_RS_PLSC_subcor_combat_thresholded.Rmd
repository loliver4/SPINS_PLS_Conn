---
title: "02_RS_PLSC_subcor_combat_thresholded"
...
RS_ts <- RS_ts[names(RS_ts) %in% include_RS$V1] # N=428
...
# keep only participants we have RS connectivity data for - N=421
spins_behav_rs <- spins_behav[spins_behav$record_id %in% names(RS_ts),]

# keep only those in EA PLS analysis - N=352
spins_behav_ea_conn <- read.csv("/projects/loliver/SPINS_PLS_Conn/data/processed/spins_behav_ea_conn_08-16-2024.csv", header = T, stringsAsFactors = F)

# keep RS data from same sample - N=352
...
spins_behav_ea_combat <- read.csv("/projects/loliver/SPINS_PLS_Conn/data/processed/spins_behav_ea_combat_08-16-2024.csv", header = T, stringsAsFactors = F)
...
#write.csv(rs_combat_data, file="/projects/loliver/SPINS_PLS_Conn/data/processed/spins_rs_combat_data_01-10-2025.csv", row.names=T)
...
# can run cross-validation after this (RS_10FoldCV.R)
...
cor.test(rs_plsc$TExPosition.Data$lx[,1],rs_plsc$TExPosition.Data$ly[,1]) # .385
...
br_beh_plot_LC1 <- PrettyBarPlot2(
  ...
  font.size = 4,
  line.alpha = 1,
  #main = 'RS LD1: Bootstrap Ratios - Behavior', 
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_beh.tiff", LD1_beh, device = "tiff", height=5.75, width=9)
...
mean_rs_cor_netorder <- lapply(rs_cor, "[", 1:392, 2:393)

# get mean corrs across participants 
mean_rs_cor_netorder <- aaply(laply(mean_rs_cor_netorder, as.matrix), c(2, 3), mean, na.rm=F)

# mirror upper and lower triangle of matrix for visualization later
mean_rs_cor_netorder[upper.tri(mean_rs_cor_netorder)] <- t(mean_rs_cor_netorder)[upper.tri(mean_rs_cor_netorder)]

# reorder rois by networks
mean_rs_cor_netorder_t <- mean_rs_cor_netorder[c(rois_netorder$orig_order), c(rois_netorder$orig_order)] 
rownames(mean_rs_cor_netorder_t) <- colnames(mean_rs_cor_netorder_t)

# melt data to create heatmap
library(reshape2)
mean_rs_cor_netorder_t_long <- melt(mean_rs_cor_netorder_t)

# add edge variables in both directions
mean_rs_cor_netorder_t_long <- unite(mean_rs_cor_netorder_t_long, col="rois", 1:2, sep="-", remove=F)
mean_rs_cor_netorder_t_long <- unite(mean_rs_cor_netorder_t_long, col="rois_rev", c(3,2), sep="-", remove=F)

# keep only connections included in PLSC - set others to 0
#dim(mean_rs_cor_netorder_t_long[ !(mean_rs_cor_netorder_t_long$rois %in% colnames(conn) | mean_rs_cor_netorder_t_long$rois_rev %in% colnames(conn)), ])
#dim(mean_rs_cor_netorder_t_long[ (mean_rs_cor_netorder_t_long$rois %in% colnames(conn) | mean_rs_cor_netorder_t_long$rois_rev %in% colnames(conn)), ])

mean_rs_cor_netorder_t_long[!(mean_rs_cor_netorder_t_long$rois %in% colnames(conn) | mean_rs_cor_netorder_t_long$rois_rev %in% colnames(conn)), "value"] <- 0
...
# plot heatmap
ggplot(data=mean_rs_cor_netorder_t_long, aes(x=factor(X1,levels=rois_netorder$forced_name),y=factor(X2,levels=rois_netorder$forced_name),fill=value)) + 
  geom_tile() +
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_origconn.png", device = "png", height=7, width=8)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_neg.png", device = "png", height=6, width=8)

data.frame(rs_conn_cont_LV1_pos[order(rs_conn_cont_LV1_pos$r, decreasing = TRUE),])
# everything is coming from the subcortex here - THA.VA.rh/THA.VA.lh (ventral anterior) are the main top contributors, and THA.DA.rh/THA.DA.lh (dorsal anterior) - almost exclusively
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_pos.png", device = "png", height=6, width=8)
...
# convert to tbl_graph tidygraph object and add other node info 
tg_rs_conn_cont_LV1_pos_nets <- as_tbl_graph(rs_conn_cont_LV1_pos_nets[,1:6], directed=F) %>%    
...
# convert to tbl_graph tidygraph object and add other node info 
tg_rs_conn_cont_LV1_neg_nets <- as_tbl_graph(rs_conn_cont_LV1_neg_nets[,1:6], directed=F) %>%    
  activate(edges) %>% mutate(val = case_when(mean>0 ~ "pos", mean<0 ~ "neg", mean==0 ~ "ns"), mean=abs(mean)) %>%
  filter(val != "ns")
...
nets_pos_LV1 <- ggraph(tg_rs_conn_cont_LV1_pos_nets, layout="linear", circular=T, sort.by=name) +
  ...
  geom_edge_loop(aes(colour=val, width=mean, span=90, direction=360), show.legend=F) + #, alpha=.6 
  ggtitle("Negative Loadings") + 
  theme_minimal() + 
  theme(panel.border = element_blank(),
        panel.grid.major = element_blank(),
  ...
  #guides(width = guide_colorbar(order = 0), fill = guide_legend(order = 1)) # tried to change legend order

nets_pos_LV1 + geom_node_text(aes(label=net_names,colour=name,size=3.5), nudge_x=.1, nudge_y=-.14, show.legend=F)
#geom_node_text(aes(label=name,colour=name), show.legend=F, nudge_x=nets_pos_LV1$data$x*.4, nudge_y=nets_pos_LV1$data$y*.2) 

#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_nets_neg.png", device = "png", height=6, width=6.5) 
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_nets_pos.png", device = "png", height=6, width=7.5)
...
ggplot(rs_conn_cont_LV1_pos_nets, aes(x=from,y=to,fill=mean)) + 
  ...
  geom_tile(data = rs_conn_cont_LV1_pos_nets[c(1,15,29,43,57,71,85,99,113,127,141,155,169),], fill = NA, color = "black", size = .5) +
  scale_fill_gradient2(midpoint=0,high = "#2166ac", mid = "white", low = "red", limit = c(-0.03,.03), name="Contribution") +  
  #scale_fill_continuous_diverging(palette = "Blue-Red", rev=T, mid=0, name="Contribution", alpha=0.7, na.value="white") + 
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_heat_nets_neg_mean.png", device = "png", height=5, width=6.5)

#V2 - slightly different colour range
ggplot(rs_conn_cont_LV1_pos_nets, aes(x=from,y=to,fill=mean)) + 
  geom_tile(color = "black") +
  geom_tile(data = rs_conn_cont_LV1_pos_nets[c(1,15,29,43,57,71,85,99,113,127,141,155,169),], fill = NA, color = "black", size = .5) +
  scale_fill_gradient2(midpoint=0,high = "#3366CC", mid = "white", low = "#FF6666", limit = c(-0.03,.03), name="Contribution") +  
  #scale_fill_continuous_diverging(palette = "Blue-Red", rev=T, mid=0, name="Contribution", alpha=0.7, na.value="white") + 
  scale_x_discrete(labels=net_names) + scale_y_discrete(labels=net_names) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.4, size = 12),
        axis.text = element_text(size = 12, color=net_cols)) + ylab("") + xlab("")
...
# heatmap mean nets pos loadings (contribution) # reversed the polarity to make it more easily comparable to EA  
ggplot(rs_conn_cont_LV1_neg_nets, aes(x=from,y=to,fill=mean)) + 
  geom_tile(color = "black") +
  geom_tile(data = rs_conn_cont_LV1_neg_nets[c(1,15,29,43,57,71,85,99,113,127,141,155,169),], fill = NA, color = "black", size = .5) +
  #scale_fill_gradient(high = "#2166ac", low = "white", limit = c(0,.03), name="") +
  scale_fill_continuous_sequential(palette = "Oslo", rev=F, begin=0, end=0.8, l1=100, name="Contribution") +
  scale_x_discrete(labels=net_names) + scale_y_discrete(labels=net_names) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.4, size = 12),
        axis.text = element_text(size = 12, color=net_cols)) + ylab("") + xlab("")
  #ggtitle("Mean Within and Between-Network Contributions") + ylab("") + xlab("")

#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_heat_nets_pos_mean.png", device = "png", height=5, width=6.5)
...
cor.test(LV1_gr[rownames(LV1_gr)=="case","Lx_conn 1"],LV1_gr[rownames(LV1_gr)=="case","Ly_beh 1"]) 
cor.test(LV1_gr[rownames(LV1_gr)=="control","Lx_conn 1"],LV1_gr[rownames(LV1_gr)=="control","Ly_beh 1"]) 
p.adjust(c(8.759e-06,0.850), method = "fdr", n = 2)
...
cor.test(LV1_gr_test[rownames(LV1_gr_test)=="control","Lx_conn 1"],LV1_gr_test[rownames(LV1_gr_test)=="control","Ly_beh 1"]) 
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_group_scatter.png", device = "png", height=4.5, width=6)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_group_brain.png", device = "png", height=4, width=5.5)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/RS_LD1_group_beh.png", device = "png", height=4, width=5.5)
...
# write out scores
#write.csv(LV_demo, file="/projects/loliver/SPINS_PLS_Conn/data/processed/spins_rs_plsc_scores.csv", row.names=F)
