...
# check if the first eigenvalue is significant - it is (p=.004)!
...
cor.test(ea_plsc$TExPosition.Data$lx[,1],ea_plsc$TExPosition.Data$ly[,1]) # .404
...
cor.test(ea_plsc$TExPosition.Data$lx[,2],ea_plsc$TExPosition.Data$ly[,2]) # .291
...
cont_beh_plot_LC1 <- PrettyBarPlot2(
  ...
  ylab = 'Signed Contributions') +
  scale_y_continuous(limits=c(0,max(abs(100*Cy_beh[,1])))) # ylim wasn't restricting the range +
...
ggplot(ea_conn_cont_LV1_neg_nets, aes(x=from,y=to,fill=mean)) + 
  geom_tile(color = "grey") +
  geom_tile(data = ea_conn_cont_LV1_pos_nets[c(1,15,29,43,57,71,85,99,113,127,141,155,169),], fill = NA, color = "black", size = .25) +
  scale_fill_gradient(high = "#2166ac", low = "white", limit = c(0,.025), name="") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 10),
        axis.text = element_text(size = 10, color=net_cols)) +
  ggtitle("Mean Within and Between-Network Contributions") + ylab("") + xlab("")
...
```{r fig.height=6, fig.width=8}
# visualize LV1 conn results conn by contribution across pos and neg loadings - no need to really do this anymore
library(tidygraph)
library(ggraph)

# change mean r values to r values x contributions for significant connections - the contributions are very small values, hence the x1000 for visualization
ea_conn_cont_LV1 <- mean_ea_conn_df
ea_conn_cont_LV1[ea_conn_cont_LV1$rois %in% names(which(ea_plsc_boot$bootRatiosSignificant.i[,1] == TRUE)), "r"] <- 1000*(ea_conn_cont_LV1[ea_conn_cont_LV1$rois %in% names(which(ea_plsc_boot$bootRatiosSignificant.i[,1] == TRUE)), "r"]*(Cx_conn[rownames(Cx_conn) %in% names(which(ea_plsc_boot$bootRatiosSignificant.i[,1] == TRUE)),1]))

# make all other connections 0
ea_conn_cont_LV1[!(ea_conn_cont_LV1$rois %in% names(which(ea_plsc_boot$bootRatiosSignificant.i[,1] == TRUE))), "r"] <- 0

# convert to tbl_graph tidygraph object and add other node info - filter out non-sig nodes
# added the numbers to neg and pos to change colour order # net_order_ColeAnt includes network number
tg_ea_conn_cont_LV1 <- as_tbl_graph(ea_conn_cont_LV1[,2:4], directed=F) %>% activate(nodes) %>% 
  mutate(roi=rois$name, lobe=as.factor(rois$lobe), Network=as.factor(rois$net_order_ColeAnt), 
  hemi=as.factor(rois$hemi), region=as.factor(rois$region)) %>%   # could change to area if update
  activate(edges) %>% mutate(val = case_when(r>0 ~ "pos", r<0 ~ "neg", r==0 ~ "ns"), r=abs(r)) %>%
  filter(val != "ns")
...
# swirly plot by network
ggraph(tg_ea_conn_cont_LV1, layout="linear", sort.by=Network) +
  geom_edge_arc(aes(colour=val,width=r),alpha=.3) + 
  geom_node_point(aes(colour=Network), size=2) +
  scale_colour_manual(values=net_cols) +
  scale_edge_width_continuous(range = c(0,1)) +
...
# circle plot by network
ggraph(tg_ea_conn_cont_LV1, layout="linear", circular=T, sort.by=Network) +
  geom_edge_arc(aes(colour=val,width=r),alpha=.3) + 
  geom_node_point(aes(colour=Network), size=4) +
  scale_colour_manual(values=net_cols) +
  scale_edge_width_continuous(range = c(0,1)) +
...
cor.test(LV1_gr[rownames(LV1_gr)=="case","Lx_conn 1"],LV1_gr[rownames(LV1_gr)=="case","Ly_beh 1"]) # case r=.408, p<.0001
cor.test(LV1_gr[rownames(LV1_gr)=="control","Lx_conn 1"],LV1_gr[rownames(LV1_gr)=="control","Ly_beh 1"]) # control r=.186, p=.019
...
cor.test(LV2_gr[rownames(LV2_gr)=="case","Lx_conn 2"],LV2_gr[rownames(LV2_gr)=="case","Ly_beh 2"]) # case r=.291, p<.0001
cor.test(LV2_gr[rownames(LV2_gr)=="control","Lx_conn 2"],LV2_gr[rownames(LV2_gr)=="control","Ly_beh 2"]) # control r=.279, p=.0004
...
p.adjust(c(.0003,.0017,.0000,.0000), method = "fdr", n = 4)
p.adjust(c(.0000,.0000,.0000,.0039), method = "fdr", n = 4)
...
p.adjust(c(.09095,.09211), method = "fdr", n = 2)
p.adjust(c(.015,.115), method = "fdr", n = 2)
