---
title: "02_EA_PLSC_subcor_combat_thresholded"
...
#write.csv(spins_behav_conn, file="/projects/loliver/SPINS_PLS_Conn/data/processed/spins_behav_ea_conn_08-16-2024.csv", row.names=F)
...
```{r}
# impute missing values for combat variables - did the first time and just reading in after 
...
#write.csv(spins_behav_combat, file="/projects/loliver/SPINS_PLS_Conn/data/processed/spins_behav_ea_combat_08-16-2024.csv", row.names=F)
...
# write out conn combat data 
#write.csv(ea_combat_data, file="/projects/loliver/SPINS_PLS_Conn/data/processed/spins_ea_combat_data_01-10-2025.csv", row.names=T)

```

```{r warning=FALSE}
# set up for PLSC (Derek Beaton's pkg; symmetric analysis as per McIntosh et al., 1996)
...
char_table <- spins_behav_combat[,c("record_id","diagnostic_group","demo_sex","demo_age_study_entry",
...
#write.csv(table,file="/projects/loliver/SPINS_PLS_Conn/reports/tables/tableone_ea_rs_imp_08-16-2024.csv")

char_table_clin <- merge(char_table,spins_behav_conn[,c("record_id","demo_eth","demo_race_white",
          "demo_race_black","demo_race_native","demo_race_asian","demo_race_islander","demo_race_other",
          "demo_race_mixed","demo_race_unknown","wtar_std_score",
          "bprs_factor_total","sans_total_sc","qls_total","bsfs_total")],by="record_id")

char_table_clin[,21:28] <- apply(char_table_clin[,21:28], 2, as.factor) # come back to this

char_table_clin <- char_table_clin[,c(1:3,20:28,4,29,5:19,30:33)]

table_one_clin <- CreateTableOne(strata="Diagnostic_Group",data=char_table_clin[,2:33])
table_clin <- print(table_one_clin)

#write.csv(table_clin,file="/projects/loliver/SPINS_PLS_Conn/reports/tables/tableone_ea_rs_imp_clin_08-16-2024.csv") # tableone_ea_rs_imp_clin_racealt_08-16-2024.csv
...
# can run cross-validation after this (EA_10FoldCV.R)
...
# check if the first eigenvalue is significant - it is (p=.001)
...
cor.test(ea_plsc$TExPosition.Data$lx[,1],ea_plsc$TExPosition.Data$ly[,1]) # .414
...
cor.test(ea_plsc$TExPosition.Data$lx[,2],ea_plsc$TExPosition.Data$ly[,2]) # .296
...
cont_conn_plot_LC1  <- PrettyBarPlot2(
            ...
            #signifOnly = TRUE,
            plotnames = TRUE,
            #main = 'EA LD1: Contributions - Connectivity',
            ylab = 'Contributions') 
...
cont_beh_plot_LC1 <- PrettyBarPlot2(
  ...
  font.size = 4,
  line.alpha = 1,
  #main = 'EA LD1: Contributions - Behavior',
  ylab = 'Contributions') +
  scale_y_continuous(limits=c(0,max(100*Cy_beh[,1]))) + # ylim wasn't restricting the range 
...
cont_beh_plot_LC2 <- PrettyBarPlot2(
  ...
  threshold = 100*(1/15),
  ylim=c(min(100*Cy_beh[,2]), max(100*Cy_beh[,2])),
  color4bar = c(rep("mediumpurple4",9),rep("darkolivegreen4",6)),
  color4ns = "gray75", 
  plotnames = TRUE,
  #font.shrink = 0,
  font.size = 4,
  line.alpha = 1,
  #main = 'EA LD2: Contributions - Behavior',
  ylab = 'Contributions') +
  scale_y_continuous(limits=c(min(100*Cy_beh[,2]), max(100*Cy_beh[,2]))) +
  theme(axis.title.y = element_text(size=13))
...
br_beh_plot_LC1 <- PrettyBarPlot2(
  ...
  font.size = 4,
  line.alpha = 1,
  #main = 'EA LD1: Bootstrap Ratios - Behavior', 
...
br_beh_plot_LC2 <- PrettyBarPlot2(
  ...
  font.size = 4,
  line.alpha = 1,
  #main = 'EA LD2: Bootstrap Ratios - Behavior', 
  ylab = "Bootstrap Ratios") +
  theme(axis.title.y = element_text(size=13))
...
sal_conn_plot_LC1 <- PrettyBarPlot2(
  ...
  #color4ns = "gray75", 
  plotnames = TRUE, 
  main = 'EA LD1: Saliences - Connectivity', 
  ylab = "Saliences")
...
sal_beh_plot_LC1 <- PrettyBarPlot2(
  ...
  plotnames = TRUE, 
  #font.size = 5,
  main = 'EA LD1: Saliences - Behavior', 
  ylab = "Saliences")
...
sal_conn_plot_LC2 <- PrettyBarPlot2(
  ...
  #color4ns = "gray75", 
  plotnames = TRUE, 
  main = 'EA LD2: Saliences - Connectivity', 
  ylab = "Saliences")
...
sal_beh_plot_LC2 <- PrettyBarPlot2(
  ...
  plotnames = TRUE, 
  #font.size = 5,
  main = 'EA LD2: Saliences - Behavior', 
  ylab = "Saliences")
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_beh.tiff", LD1_beh, device = "tiff", height=5.75, width=9)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_beh.tiff", LD2_beh, device = "tiff", height=5.75, width=9)

```


```{r}
# Visualize orig conn values 
...
rois_netorder$colour <- c(rep("darkolivegreen4",6),rep("darkorange",4),rep("brown",7),rep("red",77),rep("orchid2",15),
              rep("gold",50),rep("darkcyan",23),rep("chartreuse",23),rep("magenta4",56),rep("cyan",39),
              rep("purple",54),rep("blue",6),rep("black",32))  


# remove roi column before calculating means across matrices
mean_ea_cor_netorder <- lapply(ea_cor, "[", 1:392, 2:393)

# get mean corrs across participants 
mean_ea_cor_netorder <- aaply(laply(mean_ea_cor_netorder, as.matrix), c(2, 3), mean, na.rm=F)

# mirror upper and lower triangle of matrix for visualization later
mean_ea_cor_netorder[upper.tri(mean_ea_cor_netorder)] <- t(mean_ea_cor_netorder)[upper.tri(mean_ea_cor_netorder)]

# reorder rois by networks
mean_ea_cor_netorder_t <- mean_ea_cor_netorder[c(rois_netorder$orig_order), c(rois_netorder$orig_order)] 
rownames(mean_ea_cor_netorder_t) <- colnames(mean_ea_cor_netorder_t)

# corr plot
#rplot(mean_ea_cor_netorder_t)

# melt data to create heatmap
library(reshape2)
mean_ea_cor_netorder_t_long <- melt(mean_ea_cor_netorder_t)

# add edge variables in both directions
mean_ea_cor_netorder_t_long <- unite(mean_ea_cor_netorder_t_long, col="rois", 1:2, sep="-", remove=F)
mean_ea_cor_netorder_t_long <- unite(mean_ea_cor_netorder_t_long, col="rois_rev", c(3,2), sep="-", remove=F)

# keep only connections included in PLSC - set others to 0
#dim(mean_ea_cor_netorder_t_long[ !(mean_ea_cor_netorder_t_long$rois %in% colnames(conn) | mean_ea_cor_netorder_t_long$rois_rev %in% colnames(conn)), ])
#dim(mean_ea_cor_netorder_t_long[ (mean_ea_cor_netorder_t_long$rois %in% colnames(conn) | mean_ea_cor_netorder_t_long$rois_rev %in% colnames(conn)), ])

mean_ea_cor_netorder_t_long[!(mean_ea_cor_netorder_t_long$rois %in% colnames(conn) | mean_ea_cor_netorder_t_long$rois_rev %in% colnames(conn)), "value"] <- 0
...
# plot heatmap
ggplot(data=mean_ea_cor_netorder_t_long, aes(x=factor(X1,levels=rois_netorder$forced_name),y=factor(X2,levels=rois_netorder$forced_name),fill=value)) + 
  geom_tile() +
  #scale_fill_continuous_diverging("Blue-Red 3", na.value = "white", mid = 0, limits = c(-1,1), name="r") +
  scale_fill_gradient2(high = "#b2182b", mid = "white", low = "#2166ac", na.value = "white", midpoint = 0, limit = c(-1,1), name="r") +
  theme(axis.text.x = element_blank(), #element_text(angle = 90, hjust = 1, size = 10),
        axis.text = element_blank(), #element_text(size = 10, color=rois_netorder$colour),
        axis.ticks = element_line(color=rois_netorder$colour)) + 
  ggtitle("") + ylab("") + xlab("")

#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_origconn.png", device = "png", height=7, width=8)
...
ea_conn_sal_LV1 <- unite(ea_cor_str[[1]], col="rois", 1:2, sep="-", remove=F)
...
# multiply conn direction by absolute contribution ($ci)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_pos.png", device = "png", height=6, width=8) # png colour looks good, tiff and jpg don't
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_neg.png", device = "png", height=6, width=8)
...
ea_conn_cont_LV1_pos_nets <- ea_conn_cont_LV1_pos_df %>%
  ...
  summarise(mean=mean(r),
            mean_sig=mean(r!=0), # [r!=0] get mean based on sig values only
            count=n(),
            sum=sum(r))
...
# convert to tbl_graph tidygraph object and add other node info 
tg_ea_conn_cont_LV1_pos_nets <- as_tbl_graph(ea_conn_cont_LV1_pos_nets[,1:6], directed=F) %>%    
...
# convert to tbl_graph tidygraph object and add other node info 
tg_ea_conn_cont_LV1_neg_nets <- as_tbl_graph(ea_conn_cont_LV1_neg_nets[,1:6], directed=F) %>%    
  activate(edges) %>% mutate(val = case_when(mean>0 ~ "pos", mean<0 ~ "neg", mean==0 ~ "ns"), mean=abs(mean)) %>%
  filter(val != "ns")
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_heat_nets_pos_mean.png", device = "png", height=5, width=6.5)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_heat_nets_neg_mean.png", device = "png", height=5, width=6.5)
...
ea_conn_LV2[ea_conn_LV2$rois %in% names(which(ea_plsc_boot$bootRatiosSignificant.i[,2] == TRUE)), "r"] <- 10000*(Cx_conn[rownames(Cx_conn) %in% names(which(ea_plsc_boot$bootRatiosSignificant.i[,2] == TRUE)),2])/15
...
tg_ea_conn_LV2 <- as_tbl_graph(ea_conn_LV2[,2:4], directed=F) %>% activate(nodes) %>% 
  mutate(roi=rois$name, lobe=as.factor(rois$lobe), Network=as.factor(rois$net_order_ColeAnt), 
  hemi=as.factor(rois$hemi), region=as.factor(rois$region)) %>%   # could change to area if update
  activate(edges) %>% mutate(val = case_when(r > 0.087 ~ "pos", r < -0.087 ~ "neg", r <= 0.087 & r >= -0.087 ~ "ns"), r=abs(r)) %>%
  filter(val != "ns")
# updated to filter out vars deemed important (those that contribute more than average: 10000*(1/nrow(Cx_conn))/15
# was mutate(val = case_when(r>0 ~ "pos", r<0 ~ "neg", r==0 ~ "ns")
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_pos.png", device = "png", height=6, width=8)

data.frame(ea_conn_cont_LV2_pos[order(ea_conn_cont_LV2_pos$r, decreasing = TRUE),])
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_neg.png", device = "png", height=6, width=8)

data.frame(ea_conn_cont_LV2_neg[order(ea_conn_cont_LV2_neg$r, decreasing = TRUE),])
...
ea_conn_cont_LV2_pos_nets <- ea_conn_cont_LV2_pos_df %>%
  group_by(to, from) %>%
  summarise(mean=mean(r),  
            mean_sig=mean(r!=0), # [r!=0] get mean of sig values only 
            count=n(),
            sum=sum(r))
...
# convert to tbl_graph tidygraph object and add other node info 
tg_ea_conn_cont_LV2_pos_nets <- as_tbl_graph(ea_conn_cont_LV2_pos_nets[,1:6], directed=F) %>%    
  activate(edges) %>% mutate(val = case_when(mean>0 ~ "pos", mean<0 ~ "neg", mean==0 ~ "ns"), mean=abs(mean)) %>%
  filter(val != "ns")  
...
# convert to tbl_graph tidygraph object and add other node info 
tg_ea_conn_cont_LV2_neg_nets <- as_tbl_graph(ea_conn_cont_LV2_neg_nets[,1:6], directed=F) %>%    
  activate(edges) %>% mutate(val = case_when(mean>0 ~ "pos", mean<0 ~ "neg", mean==0 ~ "ns"), mean=abs(mean)) %>%
  filter(val != "ns")
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_nets_pos.png", device = "png", height=6, width=7.5)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_nets_neg.png", device = "png", height=6, width=7.5)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_heat_nets_pos_mean.png", device = "png", height=5, width=6.5)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_heat_nets_neg_mean.png", device = "png", height=5, width=6.5)
...
cor.test(LV1_gr[rownames(LV1_gr)=="case","Lx_conn 1"],LV1_gr[rownames(LV1_gr)=="case","Ly_beh 1"]) # case r=.409, p<.0001
cor.test(LV1_gr[rownames(LV1_gr)=="control","Lx_conn 1"],LV1_gr[rownames(LV1_gr)=="control","Ly_beh 1"]) # control r=.174, p=.0293
p.adjust(c(0.000000002918,0.0293), method = "fdr", n = 2)
...
plot1_LV1_gr <- plot_LV1_gr$zeMap_background + 
  ...
  ggtitle("") + #EA Dimension 1: Latent Variable Scores by Diagnostic Group
         labs(y = "Behavior Scores", x = "Brain Scores") +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))

print(plot1_LV1_gr)

#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_group_scatter.png", device = "png", height=4.5, width=6)
...
ggplot(LV1_gr_demo, aes( x= diagnostic_group, y = Lx_conn.1, col = diagnostic_group)) +
  geom_boxplot() + labs(y = "Brain Scores", x="SSD                     Control") + 
  ...
  theme_bw() +
     theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(),
           axis.text.x = element_blank(), axis.text=element_text(size=12), axis.title=element_text(size=14))
...
ggplot(LV1_gr_demo, aes( x= diagnostic_group, y = Lx_conn.1, fill = diagnostic_group)) + 
     ...
     labs(y = "Brain Scores", x="SSD                Control") +
     #scale_fill_manual(values = c("blue","red")) +
     theme_bw() +
     theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(),
           axis.text.x = element_blank(), axis.text=element_text(size=12), axis.title=element_text(size=14))

#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_group_brain.png", device = "png", height=4, width=5)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD1_group_beh.png", device = "png", height=4, width=5.5)
...
cor.test(LV2_gr[rownames(LV2_gr)=="case","Lx_conn 2"],LV2_gr[rownames(LV2_gr)=="case","Ly_beh 2"]) # case r=.299, p<.0001
cor.test(LV2_gr[rownames(LV2_gr)=="control","Lx_conn 2"],LV2_gr[rownames(LV2_gr)=="control","Ly_beh 2"]) # control r=.281, p=.0004
p.adjust(c(.0000219,0.00037), method = "fdr", n = 2)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_group_scatter.png", device = "png", height=4.5, width=6)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_group_brain.png", device = "png", height=4, width=5)
...
p.adjust(c(.0746,.1585), method = "fdr", n = 2)
...
#ggsave(filename="/projects/loliver/SPINS_PLS_Conn/reports/figures/EA_LD2_group_beh.png", device = "png", height=4, width=5)
...
# write out scores
#write.csv(LV_demo, file="/projects/loliver/SPINS_PLS_Conn/data/processed/spins_ea_plsc_scores.csv", row.names=F)
...
p.adjust(c(.0002,.0015,.0000,.0000), method = "fdr", n = 4)
p.adjust(c(.0000,.0000,.0000,.0052), method = "fdr", n = 4)
...
p.adjust(c(.04541,.1097), method = "fdr", n = 2)
p.adjust(c(.00568,.0751), method = "fdr", n = 2)
