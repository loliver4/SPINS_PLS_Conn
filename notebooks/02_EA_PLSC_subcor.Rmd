...
# find EA residual time series files # pattern glob2rx("*_EA_6mm_noGSR_glasser_meants.csv") # 6mm_noGSR
files_EA_resid_ts <- list.files(path= ".", recursive=T, full.names=F, pattern="^.*_EA_2mm_noGSR_glasser_tian_meants\\.csv$")
...
# CMP_0178 exclude due to postivie UDS (only determined after completion)
...
rois <- read.csv(file = "/projects/loliver/SPINS_PLS_Conn/data/Glasser_Tian_roi_info.csv", header=T)
...
# update spins_behav_conn
spins_behav_conn_pls <- cbind(spins_behav_conn[spins_behav_conn$record_id!="SPN01_ZHP_0110" & spins_behav_conn$record_id!="SPN01_ZHP_0172", c("record_id","diagnostic_group","demo_sex","demo_age_study_entry")],beh)

# generate table one
colnames(spins_behav_conn_pls) <- c("ID","Diagnostic_Group","Sex_at_birth","Age","RMET","ER40","EA","TASIT1","T2_sinc",
        "T2_simsar","T2_parsar","T3_lie","T3_sar","Proc_Speed","Att_Vig","Work_Mem","Verb_Learn","Vis_Learn","Reason_PS") 

table_one <- CreateTableOne(strata="Diagnostic_Group",data=spins_behav_conn_pls[,2:19])
...
tg_ea_conn_LV1 <- as_tbl_graph(ea_conn_LV1[,2:4], directed=F) %>% activate(nodes) %>% 
  mutate(roi=rois$atlas_roi, lobe=as.factor(rois$lobe), 
...
# swirly plot - index value refers to node location -  sig pos conns vs  sig neg conns (hence all blue)
